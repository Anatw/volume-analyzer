...
def mp3_list_to_analyze():
    return [audio_file for audio_file in os.listdir(folder_path)
             if os.path.isfile(os.path.join(folder_path, audio_file)) and audio_file.lower().endswith(".mp3")]
...
def analysed_normalized_rms_dict(return_numpy_array=False):
    ...
    if not mp3_files:
        raise FileNotFoundError(f"No '.mp3' files found in specified path: {folder_path}")
    audio_data_dict = {}
