...
def perform_stft_analysis(audio_path):
    y, sr = librosa.load(audio_path, sr=None)
    S, _ = librosa.magphase(librosa.stft(y))
    return y, sr, S


def mp3_list_to_analyze():
    return [
        audio_file
        for audio_file in os.listdir(folder_path)
        if os.path.isfile(os.path.join(folder_path, audio_file))
        and audio_file.lower().endswith(".mp3")
    ]


def analysed_normalized_rms_dict(return_numpy_array=False):
    if not os.path.isdir(folder_path):
        raise FileNotFoundError(f"Folder not found in path: {folder_path}")
    mp3_files = mp3_list_to_analyze()
    if not mp3_files:
        raise FileNotFoundError(
            f"No '.mp3' files found in specified path: {folder_path}"
        )
    audio_data_dict = {}
    numpy_normalized_rms_dict = {}
    for audio_file in mp3_files:
        audio_path = os.path.join(folder_path, audio_file)
        _, sr, S = perform_stft_analysis(audio_path)
...
def exceeding_indexes_clusters():
    ...
    _exceeded_cluster_dict = (
        {}
    )  # Inner-use dictionary with start and end values for all the clusters in each audio track.
    exceeded_cluster_dict = {}
    # Create exceeded indexes dict:
    for audio_track, audio_data_value in audio_data_dict.items():
        exceeded_indexes = [
            index
            for index, value in enumerate(audio_data_value["rms_values"])
            if value > threshold
        ]
        ...
        start = None
        distance_allowed_between_clusters = list(i for i in range(10, 101, 10)) + [
            150,
            300,
        ]
        num_clusters_added = []
        for distance in distance_allowed_between_clusters:
            ...
            for exceeded_list_index, exceeded_value in enumerate(
                exceeded_indexes_dict[audio_track]
            ):
                if not start:
                    start = exceeded_value
                    _exceeded_cluster_dict[audio_track][distance][
                        f"start{cluster_index}"
                    ] = start
                if (
                    len(exceeded_indexes_dict[audio_track]) > exceeded_list_index + 1
                    and (
            ...
            if num_clusters_created not in num_clusters_added:
                ...
                exceeded_cluster_dict[audio_track][distance] = {
                    ...
                    "num_values_in_cluster": len(
                        audio_data_dict[audio_track]["rms_values"]
                    ),
                }
        ...
        for cluster_index, (distance, clusters) in enumerate(clusters_dict.items()):
            ...
            for index in range(1, (clusters["num_clusters"] + 2)):
                if index == 1:  # If first range of the list
                    clusters_for_servo_usage[track_name][cluster_index] += [0] * (
                        clusters["clusters"][f"start{index}"]
                    )
                elif index == (
                    clusters["num_clusters"] + 1
                ):  # If last range of the list
                    clusters_for_servo_usage[track_name][cluster_index] += [0] * (
                        _exceeding_indexes_clusters[track_name][distance][
                ...
                plus = (
                    clusters["clusters"][f"end{index}"]
                    - clusters["clusters"][f"start{index}"]
                )
                clusters_for_servo_usage[track_name][cluster_index] += [1] * (
                    plus if plus > 0 else 1
                )
        # clusters_for_servo_usage[track_name][cluster_index] = rms_ready_for_servo
    # Transfer the dict into a json format:
    for audio_name_index, track_name in enumerate(audio_name_list):
        clusters_for_servo_usage[track_name]["sample_rate"] = sample_rate_list[
            audio_name_index
        ]
        # "rms_values": sound_rms_dict[track_name],
...
def generate_graphs(generate_rms=True, generate_power=False, generate_volume=False):
    ...
    for folder_path_index, audio_file in enumerate(mp3_list_to_analyze()):
        audio_path = os.path.join(folder_path, audio_file)
        ...
        if num_arguments_passed > 1:
            ...
            if not generate_rms:
                print(
                    "Can only generate a single graph with RMS values, and this is not what was requested"
                )
                return
